 ...
 *
 *   admin_access_token / admin_refresh_token   → AGENCY_OWNER  (/admin)
 *   team_access_token / team_refresh_token     → TEAM_MEMBER   (/team, /onboarding)
 *   client_access_token / client_refresh_token → SUB_ACCOUNT   (/client, /portal)
 *
...
export function getActiveRole(): SessionRole | null {
  ...
  if (path.startsWith("/client") || path.startsWith("/portal")) return "client"
